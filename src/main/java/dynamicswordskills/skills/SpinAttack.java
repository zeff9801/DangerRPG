...
public class SpinAttack extends SkillActive implements IModifiableSkill, IReachAttackSkill
{
	...
	private void startSpin(EntityPlayer player) {
		++refreshed;
		if (player.worldObj.isRemote) {
			targets = player.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, player.boundingBox.expand(getRange(), 0.0D, getRange()));//TODO check if alive
	...
	private void incrementSpin(EntityPlayer player) {
		...
		currentSpin += (float) (getSpinSpeed() * 0.21D);
		if (currentSpin >= arc) {
			deactivate(player);
