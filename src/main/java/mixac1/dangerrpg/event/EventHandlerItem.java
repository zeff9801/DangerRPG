        ...
        if (e.attacker instanceof EntityPlayer) {
            ...
            if (RPGItemHelper.isRPGable(e.stack)) {
                if (!e.isRangeed) {
                    ...
                    e.newDamage += PlayerAttributes.STRENGTH.getValue(player)
                        * ItemAttributes.STR_MUL.getSafe(e.stack, player, 0);
                }
    ...
    public void onlivingUpdate(LivingEvent.LivingUpdateEvent e) {
        if (e.entityLiving instanceof EntityPlayer) {
            ...
            if (heldItem != null && RPGItemHelper.isRPGable(heldItem) && ItemAttributes.STR_MUL.hasIt(heldItem)) {
                double newModifierValue = PlayerAttributes.STRENGTH.getValue(player)
                    * ItemAttributes.STR_MUL.get(heldItem)
                    * 2;
                AttributeModifier newMod = new AttributeModifier(
                    RPGUUIDs.ADD_STR_DAMAGE,
                    "Strength damage",
                    newModifierValue,
                    0).setSaved(true);
